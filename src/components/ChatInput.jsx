...
// ── folder row ─────────────────────────────────────────────────────────────
function FolderRow({ folder, recentFolders = [], onPickFolder, onSelectFolder, model, ollamaModels, setModel, planMode, setPlanMode, autoApply, setAutoApply, indexing, onImageClick }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
  ...
  return (
    <div style={{ position: 'relative', borderTop: '1px solid #ede9e2', padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#faf9f7', borderRadius: '0 0 16px 16px' }} ref={ref}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <button
          style={{ ...styles.paramBtn, display: 'flex', alignItems: 'center', gap: 7 }}
        ...
        <button
          style={{ ...styles.paramBtn, ...(planMode ? styles.paramBtnOn : {}), display: 'flex', alignItems: 'center', gap: 7 }}
          onClick={() => setPlanMode((p) => !p)}
          title="Plan mode: model analyses the request and outlines changes before touching any code. Reply 'go' to execute."
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <button
          style={{ ...styles.paramBtn, ...(autoApply ? styles.paramBtnOn : {}), display: 'flex', alignItems: 'center', gap: 7 }}
          onClick={() => setAutoApply((a) => { const next = !a; localStorage.setItem('codelocal-autoapply', String(next)); return next; })}
          title="Auto-apply edits (Ctrl+Shift+A): automatically apply edit blocks when the model finishes"
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
// ── chat input ─────────────────────────────────────────────────────────────
export default function ChatInput({ onSend, onStop, streaming, model, ollamaModels, setModel, folder, recentFolders, onPickFolder, onSelectFolder, planMode, setPlanMode, autoApply, setAutoApply, indexing }) {
  ...
  return (
    <div style={styles.inputWrap}>
      <div style={{ ...styles.inputBox, ...(focused ? styles.inputBoxFocused : {}), position: 'relative' }}>
        {streaming && (
          <button onClick={onStop} title="Stop generating" style={{
            position: 'absolute', top: 10, right: 10, zIndex: 1,
            display: 'flex', alignItems: 'center', gap: 6,
            background: '#ffffff', color: '#3a3a33', border: '1px solid #e0ddd6',
            borderRadius: 4, padding: '7px 14px', fontSize: 13, fontWeight: 500,
            cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,0.04)',
          }}>
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
              <rect x="1" y="1" width="9" height="9" rx="1.5" fill="#3a3a33"/>
            </svg>
            Stop
          </button>
        )}
        ...
        />
        <FolderRow folder={folder} recentFolders={recentFolders} onPickFolder={onPickFolder} onSelectFolder={onSelectFolder} model={model} ollamaModels={ollamaModels} setModel={setModel} planMode={planMode} setPlanMode={setPlanMode} autoApply={autoApply} setAutoApply={setAutoApply} indexing={indexing} />
      </div>
    </div>
  );
}
